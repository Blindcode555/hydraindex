...
**Most recent pass — the CORE INTELLIGENCE & WORKFLOW UX PATCH.** A real production bug was reported and root-caused: an unrelated physical-product idea ("create quality cat trees with interchangeable parts") produced a YouTube/Video/AI Production mission, and a second bug where an explicit "Writing" type selection was still overridden by video/YouTube behavior. Both traced to the **offline/failure fallback path** in `index.html` (`classify()` + the static `WORKFLOWS` object) — not the live OpenAI orchestrator — via two deterministic frontend bugs, now fixed; see [Root-cause fixes: the cat-tree / YouTube bug](#root-cause-fixes-the-cat-tree--youtube-bug) below. Alongside the fix, an automatic mission-level **quality gate** was added as defense in depth (see [Automatic Quality Gate](#automatic-quality-gate)), and the Mission Console got a **beginner-friendly terminology pass** (see [Beginner UX terminology pass](#beginner-ux-terminology-pass)) — presentation-only, no internal IDs/schema touched. Supabase schema, auth, RLS, the Workspace/nested-directory architecture, Ask Hydra, Stripe, and external provider APIs are all unchanged by this pass.
...
FRONTEND
  ↓
api/_lib/request-context.js   — AUTH / USER CONTEXT        (verifies a Supabase session if one is presented; anonymous otherwise)
  ↓
api/_lib/entitlements.js      — ENTITLEMENT / USAGE CHECK  (still: always allowed — plans/credits are read, not yet enforced)
  ↓
api/_lib/orchestrator.js      — OPENAI ORCHESTRATION       (strengthened this pass — see below; same models, same schema)
  ↓
api/tool-registry.json       — TOOL REGISTRY               (unchanged)
  ↓
(candidate mission JSON)
  ↓
api/_lib/mission-quality.js   — AUTOMATIC QUALITY GATE     (new this pass — ONE extra OpenAI call, fail-open; see below)
  ↓
(final mission JSON — corrected only if the gate found a real problem)
  ↓
api/_lib/mission-store.js     — SHAPE + SAVE MISSION       (per-generation record: still shaped, not persisted)
...
```
Luna / OpenAI backend
├── Mission Orchestrator   (api/generate-mission.js + api/_lib/orchestrator.js)     — prompt strengthened this pass, same function signature
├── Quality Gate           (api/_lib/mission-quality.js, called from generate-mission.js) — new this pass, its own model/prompt
├── AI Refine              (api/refine.js + api/_lib/orchestrator.js)              — unchanged
└── Ask Hydra              (api/ask-hydra.js + api/_lib/ask-hydra.js)              — unchanged this pass
```

All four share the same `OPENAI_API_KEY`, the same `getRequestContext()` auth resolution, and the same tool registry (`api/_shared.js`'s `getRegistrySummaryForPrompt()` — Ask Hydra and the Quality Gate both reuse this exact function rather than keeping a second catalog). See [Ask Hydra](#ask-hydra-a-dedicated-product-assistant) and [Automatic Quality Gate](#automatic-quality-gate) below for the full shape of each.
...
- **`api/_lib/request-context.js`** — `getRequestContext(req)` → `{ userId, isAuthenticated, plan, accessToken }`. Now does what the original version was scaffolded for: reads the `Authorization: Bearer <token>` header, verifies it against Supabase Auth (`GET /auth/v1/user`), and — if valid — looks up that user's plan from `profiles`. No token, an invalid token, or Supabase not configured all resolve to the same anonymous context as before. It still never trusts a client-supplied user id; the only identity that reaches downstream code is one Supabase itself verified.
- **`api/_lib/entitlements.js`** — unchanged this pass. `checkEntitlement(context, action)` → `{ allowed, plan, reason, limit }` still always allows; `context.plan` is now a real value (`'anonymous' | 'free' | ...`) instead of always `'anonymous'`, so real plan-based gating later is still a one-line change here, not a rewrite.
- **`api/_lib/orchestrator.js`** — same Luna models (`OPENAI_MISSION_MODEL` / `OPENAI_REFINE_MODEL`), same schema, same registry-validation, same function signatures (`generateMissionPlan`, `refineStepPrompt`). This pass strengthened `buildMissionSystemPrompt()` itself — see [Root-cause fixes](#root-cause-fixes-the-cat-tree--youtube-bug) below — and exported `buildMissionSchema` so the new quality gate can reuse the exact same mission shape rather than keeping a second, driftable copy of it.
- **`api/_lib/mission-quality.js`** — new this pass. The automatic, mission-level quality gate — see [Automatic Quality Gate](#automatic-quality-gate) below for the full design. A sibling to `orchestrator.js`, not a part of it: `generateMissionPlan()` itself is never called by this file and never calls it.
- **`api/_lib/mission-store.js`** — `buildMissionRecord`/`saveMission`/`getMissionHistory` (the per-generation, works-for-everyone path) are **unchanged and still no-ops** — generating a mission still never requires an account. New alongside them: `getProfile`, `listProjects`, `createProject`, `getProjectWithLatestMission`, `updateProjectProgress` — real Supabase reads/writes, used only by the new `/api/projects` endpoints, and only after `request-context.js` has confirmed `context.isAuthenticated`.
- **`api/_lib/supabase-server.js`** — new. A small, dependency-free wrapper around Supabase's HTTP APIs (`fetch` directly to `/auth/v1/user` and `/rest/v1/<table>`) — no `@supabase/supabase-js` on the server, so no new npm dependency and nothing to bundle. It never uses a service-role key: every request is made with either the caller's own verified access token or the public anon key, so Postgres Row Level Security (`supabase/schema.sql`) is what actually enforces per-user isolation, exactly as required.
...
## Root-cause fixes: the cat-tree / YouTube bug

**The report.** The idea "I want to create quality cat trees with interchangeable parts that can be mixed and matched and customized individually" produced a YouTube/Video/AI Production mission — semantically wrong; it should have been a Physical Product / Product Design mission. Separately, explicitly selecting the "Writing" type was still overridden by video/YouTube behavior for an unrelated idea.

**Root cause, proven not assumed.** Both symptoms traced to the client-side **offline/failure fallback** in `index.html` — `classify()` (a hardcoded keyword-scoring text classifier) plus the static `WORKFLOWS` object — which predates the OpenAI orchestrator and fires whenever `/api/generate-mission` fails or returns something unusable. This was confirmed, not guessed: the real bug report's exact wording ("Identify high-demand, low-competition topics," "Research YouTube content opportunities for…") matched **verbatim** against the hardcoded `WORKFLOWS.youtube.steps[0]` template string — a live, working OpenAI call would never reproduce a hardcoded template word-for-word. Two independent deterministic bugs in that fallback path were sufficient to explain both symptoms:

1. **Incomplete state reset.** `startNewMission()` — the function behind the sidebar's "New Mission" quick action and the nested directory's "+ New Project" row — cleared the mission output but never reset `state.type`/`state.level`/`state.budget` or the `.on` highlight on type/expertise/budget buttons. A prior mission's type selection (or its `classify()`-guessed domain) could silently survive into the next, unrelated idea. Fixed: `startNewMission()` now fully clears `state.type`/`state.level`/`state.budget`, removes every `.on` class, clears `CURRENT_MISSION`, and hides the mission output — a complete reset, not a partial one — for all three entry points (sidebar quick action, nested directory, Workspace panel), matching what `#btn-reset` already did.
2. **Wrong fallback precedence, and a bad ultimate default.** The fallback's variant-selection logic ran `classify(ideaText)` **even when an explicit type had already been selected**, so a free-text keyword guess on the idea could override the user's own explicit choice — this is exactly what turned an explicit "Writing" selection into YouTube/video behavior. And when `classify()` found no confident keyword match at all (as with the cat-tree idea, which contains none of its keywords), it silently defaulted to `'youtube'` — turning "no match" into "assume video" instead of "we don't know." Fixed, in `generateMission()`'s catch block: an explicit `state.type` is now checked **first** and always wins (via `PLANET_DEFAULTS`); only when no type was selected does the idea text get classified at all; and `classify()` itself now returns `null` (not `'youtube'`) below a minimum confidence threshold (`MIN_CLASSIFY_CONFIDENCE = 4` — below the weight of the broadest, most ambiguous catch-all keywords like "video"/"content"/"business"), which the caller then resolves to a brand-new, neutral, domain-agnostic workflow — `WORKFLOWS.general` ("Custom Project Plan": research → define the plan → build the first version → review & improve → prepare to launch → launch & share) — instead of guessing any specific medium.

**Explicitly not a bandage.** Per instruction, this was fixed as a genuine deterministic bug in the fallback's own logic — not papered over by the quality gate below. The quality gate is additional defense in depth for the **live orchestration path**; it does not run on, and cannot fix, the offline fallback, so the fallback had to be correct on its own terms.

**Also strengthened (defense in depth, not the primary fix): the live orchestrator's system prompt** (`buildMissionSystemPrompt()` in `api/_lib/orchestrator.js`) now requires an explicit reasoning chain before any node is designed — PRIMARY USER OBJECTIVE → INTENDED DELIVERABLE → PRIMARY DOMAIN → REQUIRED CAPABILITIES → APPROPRIATE TOOLS → SUPPORTING CHANNELS ONLY WHEN RELEVANT — with an explicit rule that tool availability must never drive domain selection (a video-generation tool existing in the registry must never turn a writing project into a video project), and that an explicitly-selected TYPE is a **strong constraint** on the primary domain, not a suggestion a keyword in the idea text can override. This makes the same class of mistake less likely even on a fully "successful" live API call — the actual reported bug went through the fallback, but nothing guarantees every future one will.

**Regression fixtures** (see [What was already verified](#what-was-already-verified-before-you-deploy) below): the cat-tree idea is now a permanent, named test case — via the forced-fallback path specifically, since that's the path the real incident is believed to have gone through — asserting it never produces YouTube/Video/AI Production tags; an explicit Writing selection with an unrelated idea stays Writing through the forced fallback; a genuinely video-themed idea still correctly classifies as YouTube (the fix is precedence/confidence, not a ban); and two state-leak fixtures (Video mission → New Mission → physical-product idea; SaaS mission → New Mission → Writing mission) prove no prior mission's domain survives `startNewMission()`.

## Automatic Quality Gate

A second, independent OpenAI call added as defense in depth on top of the deterministic fixes above — not a replacement for them. It checks the **entire generated mission together**, after `generateMissionPlan()` returns and before the mission is shown to the user, and can rebuild it wholesale if it's semantically wrong — not just reword a step's prompt.

**Cost/latency contract: exactly ONE additional OpenAI call per generated mission**, never one per node. A six-step mission still makes two OpenAI calls total (mission generation + one gate check), not seven.

**What it checks** (`api/_lib/mission-quality.js`'s `buildQualitySystemPrompt()`), lettered A–J: Goal Alignment, Domain Alignment, an explicit user Type Selection being respected, per-Step Relevance, per-node Tool Relevance, Sequence, Prompt Quality (specific to the real project, not generic), Contamination (leftover category/tags/tools from an unrelated kind of project), Redundancy, and Execution Truth (Hydra never implies it directly executes a tool on the user's behalf). If the candidate passes everything, it comes back with `status: "ok"` and the mission **exactly unchanged** — no paraphrasing for its own sake. If it genuinely fails one of these — most importantly goal/domain/selection/step-relevance/contamination — it comes back `status: "corrected"` with a **rebuilt** mission, e.g. a physical-product idea mistakenly turned into a video-research mission gets rebuilt around market research/design/prototyping/sourcing/pricing, not a better-worded video prompt.

**Isolated architecture, by design.** `api/_lib/mission-quality.js` is a sibling to `orchestrator.js`, not a modification of it — `generateMissionPlan()` itself is completely untouched, so Mission Orchestrator and AI Refine are provably unaffected (the existing regression suite re-runs their original happy paths after exercising the gate, and still passes). It has its own model constant, `QUALITY_MODEL` (env: `OPENAI_QUALITY_MODEL`, default `gpt-5.6-luna-quality`), reuses `orchestrator.js`'s own `buildMissionSchema()` for its output shape (so a "corrected" mission can never drift from the shape mission generation itself is held to), and reuses the same tool registry as everything else — never a second catalog, and every `tool_id` in a "corrected" mission is re-verified against the real registry exactly like a freshly-generated one is. Kept isolated enough that folding it into the primary orchestration call later (one call instead of two) is a contained follow-up, not a rewrite.

**Fail-open, absolutely.** `generate-mission.js` wraps the gate call in its own try/catch, separate from mission generation's own error handling. If the gate throws for **any** reason — no API key, network failure, a bad/malformed response, or even a "corrected" mission that itself references a tool outside the registry — the original, already-valid mission from `generateMissionPlan()` is exactly what still ships. The gate can only ever refine the *same* mission it was given, or leave it exactly as-is; it never substitutes a different, unrelated hardcoded workflow, never breaks Generate Path, and never blocks anonymous mission generation.

**What the response carries.** `/api/generate-mission`'s `meta` object now includes `quality_status` (`"quality_checked"` if the gate ran to completion — whether it confirmed the mission or corrected it — or `"quality_check_unavailable"` if it could not run, in which case the mission is exactly what `generateMissionPlan()` produced, unmodified) and `quality_corrected` (a boolean, `true` only when the gate actually rebuilt the mission) — both are internal, non-sensitive status flags only; never a reasoning string, never a technical error detail, and nothing about them is currently surfaced in the UI.

**Manual AI Refine is unchanged and conceptually distinct.** The quality gate is Hydra checking its own output automatically, once, over the whole mission; AI Refine (`api/refine.js`) is still the user manually asking Hydra to improve one specific step's prompt, on demand, per click — the two never call each other and one existing without the other still makes sense.

## Beginner UX terminology pass

Presentation-only relabeling of the Mission Console/exec-panel area, aimed at a less technical, less overwhelming first-time experience — no internal identifiers, IDs, database columns, or state variables changed (a saved project's `current_node`, a node's array index, `workflow_json`'s shape — all exactly as before; only what's painted on screen moved).

**Terminology.** Execution Path → **Your Plan**; Nodes → **Steps**; "Node 01" → **"Step 1 of 6"**; Current Node/Current Phase → **Current Step**; Structured Prompt → **Your Hydra Prompt**; Launch Tool → **Start This Step**.

**Advanced disclosure.** The exec panel's information order is now beginner-first: **What You're Doing** → (implicitly) **why**, via the step's own description → **Hydra Recommends** (the tool) → **Why This Tool?** (shown only when the AI supplied a reason) → **Estimated Time** → **Start This Step**. The full Hydra Prompt, Copy Prompt, and AI Refine — everything a beginner doesn't need up front — now live inside a collapsed-by-default `<details>`/`<summary>` **"Advanced"** disclosure beneath that. Nothing about Copy Prompt or AI Refine's own behavior changed; they just live one click deeper.

**Explicitly not done in this pass** (per instruction): no larger GPS-timeline-style redesign of the Mission Console.
...
Ask Hydra used to just forward whatever was typed into it straight to `/api/generate-mission` as an "idea" — it was Mission Orchestrator wearing a different label, with no memory, no awareness of Hydra itself, and no way to answer a question like "how do I resume my project?" It now has its own endpoint, `POST /api/ask-hydra`, backed by `api/_lib/ask-hydra.js` — a sibling to `orchestrator.js`, not a part of it, so Mission Orchestrator and AI Refine are completely unaffected.
...
api/projects/[id].js
api/config.js
api/_lib/request-context.js
api/_lib/entitlements.js
api/_lib/orchestrator.js
api/_lib/mission-quality.js
api/_lib/ask-hydra.js
api/_lib/hydra-knowledge.js
api/_lib/mission-store.js
api/_lib/supabase-server.js
```
...
### Why the helper modules live in `api/_lib/`, not `api/lib/`

Vercel's zero-config Node builder treats *every* `.js` file it finds anywhere under `/api` — recursively, including subfolders — as its own Serverless Function, unless the filename starts with an underscore (that convention is also why the tool-registry helper is `api/_shared.js`, not `api/shared.js`). Early in this project, the shared backend modules lived in a plain `api/lib/` folder. That's fine functionally — `require('./lib/...')` works the same either way — but it meant Vercel was quietly counting all 7 files in there (`orchestrator.js`, `mission-store.js`, `request-context.js`, `supabase-server.js`, `entitlements.js`, `ask-hydra.js`, `hydra-knowledge.js`) as 7 *additional* functions on top of the 6 real endpoints, for a total of 13 — one over the Hobby plan's 12-function ceiling, which is exactly the deployment error this surfaced as ("No more than 12 Serverless Functions can be added to a Deployment on the Hobby plan").

None of those files are meant to be reachable as their own HTTP endpoints — they're only ever `require()`'d by the 6 real route files (`generate-mission.js`, `refine.js`, `ask-hydra.js`, `projects.js`, `projects/[id].js`, `config.js`) and bundled into those functions' deployment package. Renaming the folder to `api/_lib/` (and updating the six `require()` paths that point into it) tells Vercel to stop treating them as functions at all — they're still bundled as dependencies of the real endpoints exactly as before, but no longer counted separately. This is a pure rename: no endpoint URL, request/response shape, Supabase/RLS setup, Luna/orchestrator behavior, environment variable, or frontend call changed.

**Result: 13 functions → 6 functions** (`generate-mission.js`, `refine.js`, `ask-hydra.js`, `projects.js`, `projects/[id].js`, `config.js` — the only files left under `/api` that aren't prefixed with `_` or a non-code asset). That leaves 6 functions of headroom below the Hobby plan's cap for future endpoints. If that headroom is ever needed again, the next-smallest step (not done in this pass) would be merging `api/projects.js` and `api/projects/[id].js` into one optional-catch-all route file — the public paths `/api/projects` and `/api/projects/:id` would stay identical either way.

**Still 6 functions after the Quality Gate.** The new `api/_lib/mission-quality.js` (added in the CORE INTELLIGENCE & WORKFLOW UX PATCH — see [Automatic Quality Gate](#automatic-quality-gate)) went straight into `api/_lib/` from day one, so it was never at risk of being counted as a 7th function. `api/_lib/` now holds 8 helper files (`orchestrator.js`, `mission-quality.js`, `mission-store.js`, `request-context.js`, `supabase-server.js`, `entitlements.js`, `ask-hydra.js`, `hydra-knowledge.js`), none of them counted — the function count was re-verified after this pass by simulating Vercel's own file-discovery rule (every `.js`/`.ts` file recursively under `/api` whose path has no `_`-prefixed segment) and confirmed still exactly **6**.
...
| `OPENAI_QUALITY_MODEL` | No | `gpt-5.6-luna-quality` | Model used by the automatic quality gate (`api/_lib/mission-quality.js`). Its own constant, distinct from all three above, so the gate can be tuned/monitored independently — see [Automatic Quality Gate](#automatic-quality-gate). |
...
- **Since the Automatic Quality Gate**: each successful `/api/generate-mission` call now makes **two** OpenAI calls total (mission generation + one quality-gate check), not one — the 12/hour figure above is still requests to the endpoint, not raw OpenAI calls, so budget roughly 2x the OpenAI spend per generation versus before this pass. The gate is capped at exactly one extra call per mission regardless of step count, by design (see [Automatic Quality Gate](#automatic-quality-gate)).

## What was already verified before you deploy

`test/e2e-test.js` drives a real headless browser against the real `index.html`, the real `workspace.html`, and the real `api/*.js` handlers — the only things mocked are the outbound call to `api.openai.com` and a stand-in Supabase (see below); no real API keys or live Supabase project are needed to run it. It's now up to **155 passing checks**, covering everything from earlier passes plus, new this pass:

- **The cat-tree/YouTube regression fixture (Test A)**: the exact reported idea, through the forced-fallback path (the real incident's believed route), must not produce YouTube/Video/AI Production tags and must land on the new neutral `general` workflow instead.
- **The Writing-override regression fixture (Test B)**: an explicit Writing type selection with that same unrelated idea, through the forced fallback, must stay Writing (Newsletter/Blog/Email tags), never overridden by video/YouTube behavior.
- **Classifier precision (Test C)**: a genuinely video-themed idea ("Create a YouTube channel reviewing modular cat furniture") must still correctly classify as YouTube through the forced fallback — proving the fix is precedence/confidence, not an outright ban on ever selecting YouTube.
- **State-leak fixtures (Tests D & E)**: a Video/YouTube mission, then `startNewMission()` (called directly, the exact function the bug traced to), then an unrelated physical-product idea with no type selected, must carry zero leftover Video/YouTube state and land on `general`; symmetrically, a SaaS mission followed by a Writing mission must carry zero leftover SaaS/MVP/GTM tags.
- **Beginner UX terminology pass**: a node card and the exec panel header both show "STEP 1 OF N" language; the Advanced disclosure is collapsed by default and the full Hydra Prompt is hidden until it's expanded; Start This Step, Copy Prompt, and AI Refine are all still present and functional once Advanced is opened.
...
`test/smoke-test.js` separately checks the backend pipeline's edge cases in isolation — **55 passing checks**, up from 42, adding a dedicated section for the automatic quality gate: a mission the gate confirms ("ok") ships completely unchanged and makes **exactly one** additional OpenAI call beyond mission generation (proven by counting outbound model names, not just asserting it); a mission the gate rebuilds ("corrected") ships the corrected version, re-enriched from the real tool registry, with `quality_corrected: true` recorded in the response's `meta`; a "corrected" mission that itself references an unregistered tool_id is rejected and the original mission still ships; an upstream error or a network failure on the gate's own call both fail open with the original mission shipping and no crash; the gate's model constant is verified distinct from Mission Orchestrator's, AI Refine's, and Ask Hydra's; and a final regression check re-runs both Mission Orchestrator's and AI Refine's original happy paths after all of the above to confirm the gate didn't disturb either. (The classification/fallback fixes and the beginner UX pass are both pure front-end logic and rendering, so they're covered end-to-end in `test/e2e-test.js` above rather than duplicated here.)
